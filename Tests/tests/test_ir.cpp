...
bool RunIrTextGcRuntimeOpsTest() {
  const char* text =
      ...
      "end\n"
      "entry main\n";
  auto module = BuildIrTextModule(text, "ir_text_gc_runtime_ops");
  if (module.empty()) return false;
  auto load = Simple::Byte::LoadModuleFromBytes(module);
  if (!load.ok) return false;
  bool saw_safepoint = false;
  bool saw_alloc = false;
  bool saw_keepalive = false;
  for (uint8_t byte : load.module.code) {
    saw_safepoint = saw_safepoint || byte == static_cast<uint8_t>(Simple::Byte::OpCode::Safepoint);
    saw_alloc = saw_alloc || byte == static_cast<uint8_t>(Simple::Byte::OpCode::AllocCheckpoint);
    saw_keepalive = saw_keepalive || byte == static_cast<uint8_t>(Simple::Byte::OpCode::KeepAlive);
  }
  return saw_safepoint && saw_alloc && saw_keepalive && RunExpectExit(module, 4);
}
...
bool RunIrTextChannelMarkerBadTest() {
  const char* text =
      "func main locals=0 stack=4\n"
      "  enter 0\n"
      "  const i32 1\n"
      "  channel.recv Missing\n"
      "  ret\n"
      "end\n"
      "entry main\n";
...
bool RunIrTextCheckedArithmeticU32Test() {
  const char* text =
      ...
      "end\n"
      "entry main\n";
  auto module = BuildIrTextModule(text, "ir_text_checked_arithmetic_u32");
  if (module.empty()) return false;
  return RunExpectExit(module, 1);
}

bool RunIrTextCheckedArithmeticU32TrapTest() {
  const char* add_text =
      "func main locals=0 stack=8\n"
      "  enter 0\n"
      ...
      "end\n"
      "entry main\n";
  auto add_module = BuildIrTextModule(add_text, "ir_text_checked_add_u32_trap");
  if (add_module.empty() || !RunExpectTrap(add_module, "ir_text_checked_add_u32_trap")) return false;
  const char* div_text =
      "func main locals=0 stack=8\n"
      "  enter 0\n"
      "  const u32 1\n"
      ...
      "end\n"
      "entry main\n";
  auto div_module = BuildIrTextModule(div_text, "ir_text_checked_div_u32_trap");
  if (div_module.empty()) return false;
  return RunExpectTrap(div_module, "ir_text_checked_div_u32_trap");
}

bool RunIrTextCheckedArithmeticDivModI64Test() {
  const char* text =
      "func main locals=0 stack=8\n"
      "  enter 0\n"
      "  const i64 85\n"
      ...
      "end\n"
      "entry main\n";
  auto module = BuildIrTextModule(text, "ir_text_checked_arithmetic_div_mod_i64");
  if (module.empty()) return false;
  return RunExpectExit(module, 42);
}

bool RunIrTextCheckedArithmeticDivModI64TrapTest() {
  const char* div_zero_text =
      ...
      "end\n"
      "entry main\n";
  auto div_zero_module = BuildIrTextModule(div_zero_text, "ir_text_checked_div_i64_zero_trap");
  if (div_zero_module.empty() || !RunExpectTrap(div_zero_module, "ir_text_checked_div_i64_zero_trap")) return false;
  const char* mod_zero_text =
      ...
      "end\n"
      "entry main\n";
  auto mod_zero_module = BuildIrTextModule(mod_zero_text, "ir_text_checked_mod_i64_zero_trap");
  if (mod_zero_module.empty()) return false;
  return RunExpectTrap(mod_zero_module, "ir_text_checked_mod_i64_zero_trap");
}

bool RunIrTextCheckedArithmeticMulI64Test() {
  const char* text =
      "func main locals=0 stack=8\n"
      "  enter 0\n"
      "  const i64 6\n"
      ...
      "end\n"
      "entry main\n";
  auto module = BuildIrTextModule(text, "ir_text_checked_arithmetic_mul_i64");
  if (module.empty()) return false;
  return RunExpectExit(module, 42);
}

bool RunIrTextCheckedArithmeticMulI64OverflowTrapTest() {
  const char* text =
      "func main locals=0 stack=8\n"
      "  enter 0\n"
      "  const i64 4611686018427387904\n"
      "  const i64 2\n"
      "  checked.mul.i64\n"
      "  conv i64 i32\n"
      "  ret\n"
      "end\n"
      "entry main\n";
  auto module = BuildIrTextModule(text, "ir_text_checked_arithmetic_mul_i64_overflow_trap");
  if (module.empty()) return false;
  return RunExpectTrap(module, "ir_text_checked_arithmetic_mul_i64_overflow_trap");
}

bool RunIrTextCheckedArithmeticSubI64Test() {
  const char* text =
      "func main locals=0 stack=8\n"
      "  enter 0\n"
      "  const i64 50\n"
      ...
      "end\n"
      "entry main\n";
  auto module = BuildIrTextModule(text, "ir_text_checked_arithmetic_sub_i64");
  if (module.empty()) return false;
  return RunExpectExit(module, 42);
}

bool RunIrTextCheckedArithmeticSubI64OverflowTrapTest() {
  const char* text =
      "func main locals=0 stack=8\n"
      "  enter 0\n"
      ...
      "end\n"
      "entry main\n";
  auto module = BuildIrTextModule(text, "ir_text_checked_arithmetic_sub_i64_overflow_trap");
  if (module.empty()) return false;
  return RunExpectTrap(module, "ir_text_checked_arithmetic_sub_i64_overflow_trap");
}
...
bool RunIrTextCheckedListF32Test() {
  const char* text =
      ...
      "  const i32 0\n"
      "  const f32 42.0\n"
      "  checked.list.set.f32\n"
      "  ldloc 0\n"
      "  const i32 0\n"
      "  checked.list.get f32\n"
      "  conv f32 i32\n"
      "  ret\n"
      "end\n"
      "entry main\n";
  auto module = BuildIrTextModule(text, "ir_text_checked_list_f32");
  if (module.empty()) return false;
  return RunExpectExit(module, 42);
}

bool RunIrTextCheckedListI64Test() {
  const char* text =
      "func main locals=1 stack=16\n"
      "  enter 1\n"
      "  newlist i64 0\n"
      ...
      "  const i32 0\n"
      "  const i64 42\n"
      "  checked.list.set.i64\n"
      "  ldloc 0\n"
      "  const i32 0\n"
      "  checked.list.get i64\n"
      "  conv i64 i32\n"
      "  ret\n"
      "end\n"
      "entry main\n";
  auto module = BuildIrTextModule(text, "ir_text_checked_list_i64");
  if (module.empty()) return false;
  return RunExpectExit(module, 42);
}
...
bool RunIrTextCheckedArrayF32Test() {
  const char* text =
      ...
      "  const i32 1\n"
      "  const f32 42.0\n"
      "  checked.array.set.f32\n"
      "  ldloc 0\n"
      "  const i32 1\n"
      "  checked.array.get f32\n"
      "  conv f32 i32\n"
      "  ret\n"
      "end\n"
      "entry main\n";
  auto module = BuildIrTextModule(text, "ir_text_checked_array_f32");
  if (module.empty()) return false;
  return RunExpectExit(module, 42);
}

bool RunIrTextCheckedArrayI64Test() {
  const char* text =
      "func main locals=1 stack=16\n"
      "  enter 1\n"
      "  newarray i64 2\n"
      ...
      "  const i32 1\n"
      "  const i64 42\n"
      "  checked.array.set.i64\n"
      "  ldloc 0\n"
      "  const i32 1\n"
      "  checked.array.get i64\n"
      "  conv i64 i32\n"
      "  ret\n"
      "end\n"
      "entry main\n";
  auto module = BuildIrTextModule(text, "ir_text_checked_array_i64");
  if (module.empty()) return false;
  return RunExpectExit(module, 42);
}

bool RunIrTextCheckedArrayI32TrapTest() {
  const char* text =
      "func main locals=1 stack=8\n"
      "  enter 1\n"
      "  newarray i32 1\n"
      ...
      "end\n"
      "entry main\n";
  auto module = BuildIrTextModule(text, "ir_text_checked_array_i32_trap");
  if (module.empty()) return false;
  return RunExpectTrap(module, "ir_text_checked_array_i32_trap");
}
...
static const TestCase kIrTests[] = {
  ...
  {"ir_text_checked_arithmetic_div_mod_i64", RunIrTextCheckedArithmeticDivModI64Test},
  {"ir_text_checked_arithmetic_div_mod_i64_trap", RunIrTextCheckedArithmeticDivModI64TrapTest},
  {"ir_text_checked_arithmetic_mul_i64", RunIrTextCheckedArithmeticMulI64Test},
  {"ir_text_checked_arithmetic_mul_i64_overflow_trap", RunIrTextCheckedArithmeticMulI64OverflowTrapTest},
  {"ir_text_checked_arithmetic_sub_i64", RunIrTextCheckedArithmeticSubI64Test},
  {"ir_text_checked_arithmetic_sub_i64_overflow_trap", RunIrTextCheckedArithmeticSubI64OverflowTrapTest},
