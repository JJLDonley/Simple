    ...
    while (pc < end) {
      ...
      if (is_extended_opcode) {
        switch (ext_opcode) {
          ...
          case Simple::Byte::ExtendedOpCode::CheckedModI32: {
            const char* label = "CHECKED_MUL_I32 type mismatch";
            if (ext_opcode == Simple::Byte::ExtendedOpCode::CheckedAddI32) {
              label = "CHECKED_ADD_I32 type mismatch";
            } else if (ext_opcode == Simple::Byte::ExtendedOpCode::CheckedSubI32) {
              label = "CHECKED_SUB_I32 type mismatch";
            } else if (ext_opcode == Simple::Byte::ExtendedOpCode::CheckedDivI32) {
              label = "CHECKED_DIV_I32 type mismatch";
            } else if (ext_opcode == Simple::Byte::ExtendedOpCode::CheckedModI32) {
              label = "CHECKED_MOD_I32 type mismatch";
            }
            ValType b = pop_type();
            ValType a = pop_type();
          ...
          case Simple::Byte::ExtendedOpCode::CheckedModI64: {
            ...
            } else if (ext_opcode == Simple::Byte::ExtendedOpCode::CheckedDivI64) {
              label = "CHECKED_DIV_I64 type mismatch";
            } else if (ext_opcode == Simple::Byte::ExtendedOpCode::CheckedModI64) {
              label = "CHECKED_MOD_I64 type mismatch";
            }
            ValType b = pop_type();
            ValType a = pop_type();
            VerifyResult r1 = check_type(a, ValType::I64, label);
            if (!r1.ok) return r1;
            VerifyResult r2 = check_type(b, ValType::I64, label);
            if (!r2.ok) return r2;
          ...
          case Simple::Byte::ExtendedOpCode::CheckedModU32: {
            ValType b = pop_type();
            ValType a = pop_type();
            VerifyResult r1 = check_type(a, ValType::U32, "CHECKED_U32 type mismatch");
            if (!r1.ok) return r1;
            VerifyResult r2 = check_type(b, ValType::U32, "CHECKED_U32 type mismatch");
            if (!r2.ok) return r2;
          ...
          case Simple::Byte::ExtendedOpCode::CheckedArrayGetI32: {
            ...
            VerifyResult r1 = check_type(arr, ValType::Ref, "CHECKED_ARRAY_GET_I32 ref type mismatch");
            if (!r1.ok) return r1;
            VerifyResult r2 = check_type(idx, ValType::I32, "CHECKED_ARRAY_GET_I32 index type mismatch");
            if (!r2.ok) return r2;
            push_type(ValType::I32);
            pc = next;
            continue;
          ...
          case Simple::Byte::ExtendedOpCode::CheckedArraySetI32: {
            ...
            ValType idx = pop_type();
            ValType arr = pop_type();
            VerifyResult r1 = check_type(arr, ValType::Ref, "CHECKED_ARRAY_SET_I32 ref type mismatch");
            if (!r1.ok) return r1;
            VerifyResult r2 = check_type(idx, ValType::I32, "CHECKED_ARRAY_SET_I32 index type mismatch");
            if (!r2.ok) return r2;
            VerifyResult r3 = check_type(value, ValType::I32, "CHECKED_ARRAY_SET_I32 value type mismatch");
            if (!r3.ok) return r3;
            pc = next;
            continue;
          }
          case Simple::Byte::ExtendedOpCode::CheckedArrayGetI64: {
            ValType idx = pop_type();
            ValType arr = pop_type();
            VerifyResult r1 = check_type(arr, ValType::Ref, "CHECKED_ARRAY_GET_I64 ref type mismatch");
            if (!r1.ok) return r1;
            VerifyResult r2 = check_type(idx, ValType::I32, "CHECKED_ARRAY_GET_I64 index type mismatch");
            if (!r2.ok) return r2;
            push_type(ValType::I64);
            pc = next;
            continue;
          ...
          case Simple::Byte::ExtendedOpCode::CheckedArraySetI64: {
            ...
            ValType idx = pop_type();
            ValType arr = pop_type();
            VerifyResult r1 = check_type(arr, ValType::Ref, "CHECKED_ARRAY_SET_I64 ref type mismatch");
            if (!r1.ok) return r1;
            VerifyResult r2 = check_type(idx, ValType::I32, "CHECKED_ARRAY_SET_I64 index type mismatch");
            if (!r2.ok) return r2;
            VerifyResult r3 = check_type(value, ValType::I64, "CHECKED_ARRAY_SET_I64 value type mismatch");
            if (!r3.ok) return r3;
            pc = next;
            continue;
          }
          case Simple::Byte::ExtendedOpCode::CheckedArrayGetF32: {
            ValType idx = pop_type();
            ValType arr = pop_type();
            VerifyResult r1 = check_type(arr, ValType::Ref, "CHECKED_ARRAY_GET_F32 ref type mismatch");
            if (!r1.ok) return r1;
            VerifyResult r2 = check_type(idx, ValType::I32, "CHECKED_ARRAY_GET_F32 index type mismatch");
            if (!r2.ok) return r2;
          ...
          case Simple::Byte::ExtendedOpCode::CheckedArraySetF32: {
            ...
            ValType idx = pop_type();
            ValType arr = pop_type();
            VerifyResult r1 = check_type(arr, ValType::Ref, "CHECKED_ARRAY_SET_F32 ref type mismatch");
            if (!r1.ok) return r1;
            VerifyResult r2 = check_type(idx, ValType::I32, "CHECKED_ARRAY_SET_F32 index type mismatch");
            if (!r2.ok) return r2;
            VerifyResult r3 = check_type(value, ValType::F32, "CHECKED_ARRAY_SET_F32 value type mismatch");
            if (!r3.ok) return r3;
            pc = next;
            continue;
          }
          case Simple::Byte::ExtendedOpCode::CheckedArrayGetF64: {
            ValType idx = pop_type();
            ValType arr = pop_type();
            VerifyResult r1 = check_type(arr, ValType::Ref, "CHECKED_ARRAY_GET_F64 ref type mismatch");
            if (!r1.ok) return r1;
            VerifyResult r2 = check_type(idx, ValType::I32, "CHECKED_ARRAY_GET_F64 index type mismatch");
            if (!r2.ok) return r2;
          ...
          case Simple::Byte::ExtendedOpCode::CheckedArraySetF64: {
            ...
            ValType idx = pop_type();
            ValType arr = pop_type();
            VerifyResult r1 = check_type(arr, ValType::Ref, "CHECKED_ARRAY_SET_F64 ref type mismatch");
            if (!r1.ok) return r1;
            VerifyResult r2 = check_type(idx, ValType::I32, "CHECKED_ARRAY_SET_F64 index type mismatch");
            if (!r2.ok) return r2;
            VerifyResult r3 = check_type(value, ValType::F64, "CHECKED_ARRAY_SET_F64 value type mismatch");
            if (!r3.ok) return r3;
            pc = next;
            continue;
          }
          case Simple::Byte::ExtendedOpCode::CheckedArrayGetRef: {
            ValType idx = pop_type();
            ValType arr = pop_type();
            VerifyResult r1 = check_type(arr, ValType::Ref, "CHECKED_ARRAY_GET_REF ref type mismatch");
            if (!r1.ok) return r1;
            VerifyResult r2 = check_type(idx, ValType::I32, "CHECKED_ARRAY_GET_REF index type mismatch");
            if (!r2.ok) return r2;
          ...
          case Simple::Byte::ExtendedOpCode::CheckedArraySetRef: {
            ...
            ValType idx = pop_type();
            ValType arr = pop_type();
            VerifyResult r1 = check_type(arr, ValType::Ref, "CHECKED_ARRAY_SET_REF ref type mismatch");
            if (!r1.ok) return r1;
            VerifyResult r2 = check_type(idx, ValType::I32, "CHECKED_ARRAY_SET_REF index type mismatch");
            if (!r2.ok) return r2;
            VerifyResult r3 = check_type(value, ValType::Ref, "CHECKED_ARRAY_SET_REF value type mismatch");
            if (!r3.ok) return r3;
            pc = next;
            continue;
          }
          case Simple::Byte::ExtendedOpCode::CheckedListGetI32: {
            ...
            VerifyResult r1 = check_type(list, ValType::Ref, "CHECKED_LIST_GET_I32 ref type mismatch");
            if (!r1.ok) return r1;
            VerifyResult r2 = check_type(idx, ValType::I32, "CHECKED_LIST_GET_I32 index type mismatch");
            if (!r2.ok) return r2;
            push_type(ValType::I32);
            pc = next;
            continue;
          ...
          case Simple::Byte::ExtendedOpCode::CheckedListSetI32: {
            ...
            ValType idx = pop_type();
            ValType list = pop_type();
            VerifyResult r1 = check_type(list, ValType::Ref, "CHECKED_LIST_SET_I32 ref type mismatch");
            if (!r1.ok) return r1;
            VerifyResult r2 = check_type(idx, ValType::I32, "CHECKED_LIST_SET_I32 index type mismatch");
            if (!r2.ok) return r2;
            VerifyResult r3 = check_type(value, ValType::I32, "CHECKED_LIST_SET_I32 value type mismatch");
            if (!r3.ok) return r3;
            pc = next;
            continue;
          }
          case Simple::Byte::ExtendedOpCode::CheckedListGetI64: {
            ValType idx = pop_type();
            ValType list = pop_type();
            VerifyResult r1 = check_type(list, ValType::Ref, "CHECKED_LIST_GET_I64 ref type mismatch");
            if (!r1.ok) return r1;
            VerifyResult r2 = check_type(idx, ValType::I32, "CHECKED_LIST_GET_I64 index type mismatch");
            if (!r2.ok) return r2;
            push_type(ValType::I64);
            pc = next;
            continue;
          ...
          case Simple::Byte::ExtendedOpCode::CheckedListSetI64: {
            ...
            ValType idx = pop_type();
            ValType list = pop_type();
            VerifyResult r1 = check_type(list, ValType::Ref, "CHECKED_LIST_SET_I64 ref type mismatch");
            if (!r1.ok) return r1;
            VerifyResult r2 = check_type(idx, ValType::I32, "CHECKED_LIST_SET_I64 index type mismatch");
            if (!r2.ok) return r2;
            VerifyResult r3 = check_type(value, ValType::I64, "CHECKED_LIST_SET_I64 value type mismatch");
            if (!r3.ok) return r3;
            pc = next;
            continue;
          }
      ...
      switch (static_cast<OpCode>(opcode)) {
        ...
        case OpCode::ListResize: {
          pop_type(); // fill value is checked dynamically against the concrete list element representation.
          ValType size = pop_type();
          ValType list = pop_type();
          VerifyResult r1 = check_type(list, ValType::Ref, "LIST_RESIZE type mismatch");
          if (!r1.ok) return r1;
          VerifyResult r2 = check_type(size, ValType::I32, "LIST_RESIZE size type mismatch");
          if (!r2.ok) return r2;
          break;
        }
        ...
        case OpCode::StringFind: {
          ...
          if (!r2.ok) return r2;
          push_type(opcode == static_cast<uint8_t>(OpCode::StringEq) ||
                    opcode == static_cast<uint8_t>(OpCode::StringNe) ? ValType::Bool : ValType::I32);
          break;
        }
