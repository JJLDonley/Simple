...
bool WriteFileAtomicallyIfMissing(const std::filesystem::path& path,
  ...
  const fs::path temporary =
      ...
      (path.filename().string() + ".tmp." +
       std::to_string(Simple::Platform::CurrentProcessId()) + "." +
...
bool WriteAutoModuleMapIfMissing(const std::filesystem::path& project_root,
  ...
  contents << "// Auto-generated by simplevm. Maps module names to .simple files.\n";
  for (const auto& [name, path] : entries) {
    std::error_code ec;
    fs::path rel = fs::relative(path, project_root, ec);
    if (ec || rel.empty()) rel = path;
    contents << name << "=\"" << rel.generic_string() << "\"\n";
  }
  return WriteFileAtomicallyIfMissing(map_path, contents.str());
}
