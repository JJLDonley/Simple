...
bool CompileSimpleToSbc(const std::string& source,
                        const std::string& name,
                        std::vector<uint8_t>* out,
                        std::string* error) {
  std::string sir;
...
int RunSimpleFile(const std::string& path, bool verify) {
  ...
  if (!ReadFileText(path, &text, &error)) {
    std::cerr << "simple load failed: " << path << " (" << error << ")\n";
    return 2;
  }
...
int RunSimplePerfDir(const std::string& dir, size_t iterations, bool verify) {
  ...
  if (files.empty()) {
    std::cerr << "no .simple files in " << dir << "\n";
    return 2;
  }
  ...
  for (const auto& path : files) {
    ...
    for (size_t i = 0; i < iterations; ++i) {
      ...
      if (!ReadFileText(path.string(), &text, &error)) {
        std::cerr << "simple load failed (" << path.filename().string() << "): " << error << "\n";
        return 2;
      }
      std::vector<uint8_t> bytes;
      if (!CompileSimpleToSbc(text, path.string(), &bytes, &error)) {
        std::cerr << "simple compile failed (" << path.filename().string() << "): " << error << "\n";
        return 2;
      }
      last_exit = RunSbcBytes(bytes, verify, &error);
      if (!error.empty() && last_exit != 0) {
        std::cerr << "simple run failed (" << path.filename().string() << "): " << error << "\n";
        return 2;
      }
    }
