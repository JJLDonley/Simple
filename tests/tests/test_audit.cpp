...
bool AuditNoStaleLibraryDiagnosticNames() {
  ...
  bool ok = true;
  for (const char* root : {"source/Lang", "source/VM", "source/LSP", "source/CLI", "tests/tests"}) {
...
bool AuditCanonicalLibraryComparisonsStayAtBoundaries() {
  ...
  const std::vector<std::string> roots = {"source/Lang", "source/VM", "source/Byte",
                                          "source/Library", "source/LSP", "source/CLI"};
  bool ok = true;
  for (const std::string& root : roots) {
    for (const auto& path : CollectFiles(root, ".cpp")) {
      const std::string normalized = NormalizePath(path);
      const bool allowed_boundary =
          normalized.find("source/LSP/") == 0 || normalized.find("source/CLI/") == 0 ||
          normalized.find("source/Lang/src/CAST/") == 0 ||
          normalized.find("source/Lang/src/Lexer/") == 0 ||
          normalized == "source/VM/src/runtime/import_dispatch.cpp" ||
          normalized.find("source/Library/") == 0;
...
bool AuditNativeSpecsUseBuilderBoundary() {
  ...
  bool ok = true;
  for (const auto& path : CollectFiles("source/VM/src/native", ".cpp")) {
    const std::string normalized = NormalizePath(path);
