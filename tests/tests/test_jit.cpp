...
std::vector<uint8_t> BuildTypesI32RefStringI64ExternalPtrs() {
  std::vector<uint8_t> types = BuildTypesI32RefStringI64();
  auto append_pointer = [&](uint8_t flags) {
    AppendU32(types, 0);
  ...
  append_pointer(Simple::Byte::kTypeFlagPointerExternal |
...
bool RunLlvmJitDynamicDlContextHelperInsideLoopTest() {
  ...
  Simple::Byte::LoadResult load = Simple::Byte::LoadModuleFromBytes(
      BuildSingleImportFunctionModuleWithTypes(main_code, 1, "System.FFI", "call$i64",
                                               SigSpec{5, 2, {4, 5}}, BuildTypesI32RefStringI64ExternalPtrs()));
  if (!load.ok) {
    std::cerr << "load failed: " << load.error << "\n";
    return false;
  }
...
bool RunLlvmJitDynamicDlManagedSignatureDiagnosticTest() {
  ...
  Simple::Byte::LoadResult load = Simple::Byte::LoadModuleFromBytes(
      BuildSingleImportFunctionModuleWithTypes(main_code, 1, "System.FFI", "call$managed",
                                               SigSpec{0, 2, {4, 1}}, BuildTypesI32RefStringI64ExternalPtrs()));
  if (!load.ok) {
    std::cerr << "load failed: " << load.error << "\n";
    return false;
  }
