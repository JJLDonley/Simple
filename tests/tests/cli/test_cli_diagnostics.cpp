...
bool CliCheckSimpleErrorFormat() {
  ...
  const std::string contents =
      RunCliDiagnosticsCaptureStderr(CliSvmCommand("check tests/simple_bad/unknown_identifier.simple"), &exit_code);
...
bool CliCheckSimpleLexerErrorFormat() {
  int exit_code = -1;
  const std::string contents =
      RunCliDiagnosticsCaptureStderr(CliSvmCommand("check tests/simple_bad/lexer_invalid_char.simple"), &exit_code);
...
bool CliCheckSimpleParserErrorFormat() {
  int exit_code = -1;
  const std::string contents =
      RunCliDiagnosticsCaptureStderr(CliSvmCommand("check tests/simple_bad/parser_unterminated_block.simple"), &exit_code);
...
bool CliStderrDiagnosticContract() {
  int exit_code = -1;
  std::string stderr_text = RunCliDiagnosticsCaptureStderr(CliSvmCommand("check tests/simple/hello.simple"), &exit_code);
  if (exit_code != 0 || !stderr_text.empty()) return false;

  stderr_text = RunCliDiagnosticsCaptureStderr(CliSvmCommand("check tests/simple_bad/type_mismatch.simple"), &exit_code);
...
bool CliResolverDiagnosticCode() {
  ...
  std::string stderr_text =
      RunCliDiagnosticsCaptureStderr(CliSvmCommand("check tests/simple_bad/module_unknown_member.simple"), &exit_code);
...
bool CliMissingInputDiagnostics() {
  int exit_code = -1;
  std::string stderr_text = RunCliDiagnosticsCaptureStderr(CliSvmCommand("check"), &exit_code);
  ...
  stderr_text = RunCliDiagnosticsCaptureStderr(
      CliSvmCommand("check \"" + missing_path.string() + "\""), &exit_code);
  return exit_code == 1 &&
