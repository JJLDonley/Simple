...
bool DecodeSemanticData(const std::string& out, std::vector<SemanticTokenEntry>* entries) {
  if (!entries) return false;
  std::vector<int> data;
  if (!ExtractSemanticData(out, &data)) return false;
  ...
  for (size_t i = 0; i + 4 < data.size(); i += 5) {
    line += data[i];
    if (data[i] == 0) col += data[i + 1];
    else col = data[i + 1];
    SemanticTokenEntry entry;
    entry.line = line;
    entry.col = col;
    entry.len = data[i + 2];
    entry.type = data[i + 3];
    entry.modifiers = data[i + 4];
...
bool LspInitializeHandshake() {
  ...
  return err_contents.empty() &&
         ...
         out_contents.find("\"declarationProvider\":true") != std::string::npos &&
...
bool LspDidOpenAcceptsLambdas() {
  const std::string session_name = "simple_lsp_lambda_diag";
  const std::string uri = "file:///workspace/lambda.simple";
  const std::string init_req = "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"initialize\",\"params\":{}}";
  const std::string open_req =
      "{\"jsonrpc\":\"2.0\",\"method\":\"textDocument/didOpen\",\"params\":{\"textDocument\":{"
      "\"uri\":\"" + uri + "\",\"languageId\":\"simple\",\"version\":1,"
...
bool LspDidOpenResolvesLocalFileImport() {
  ...
  return err_contents.empty() &&
         ...
         out_contents.find("\"uri\":\"" + uri + "\"") != std::string::npos &&
         out_contents.find("\"diagnostics\":[]") != std::string::npos &&
         out_contents.find("unsupported import path") == std::string::npos &&
         out_contents.find("\"code\":\"E0001\"") == std::string::npos;
}
...
bool LspDiagnosticsStripWrappedContextAndUseInnerRange() {
  ...
  return err_contents.empty() &&
         ...
         out_contents.find("\"end\":{\"line\":1,\"character\":8}") != std::string::npos;
}
...
bool LspHoverIncludesParameterUseType() {
  ...
  const std::string open_req =
      "{\"jsonrpc\":\"2.0\",\"method\":\"textDocument/didOpen\",\"params\":{\"textDocument\":{"
      "\"uri\":\"" + uri + "\",\"languageId\":\"simple\",\"version\":1,"
      "\"text\":\"draw : (x : i32, count :: i32) -> void {\\n  count;\\n}\"}}}";
...
bool LspHoverShowsSimpleFunctionSignatureSyntax() {
  ...
  const std::string open_req =
      "{\"jsonrpc\":\"2.0\",\"method\":\"textDocument/didOpen\",\"params\":{\"textDocument\":{"
      "\"uri\":\"" + uri + "\",\"languageId\":\"simple\",\"version\":1,"
      "\"text\":\"add : (lhs : i32, rhs :: i32) -> i32 { return lhs; }\\nmain : () -> i32 { return add(1, 2); }\"}}}";
  const std::string hover_decl =
  ...
  return ReadFileText(err_path).empty() && out_contents.find("\"id\":73") != std::string::npos &&
         out_contents.find("add : (lhs : i32, rhs :: i32) -> i32") != std::string::npos &&
...
bool LspHoverShowsAsyncFunctionAndPromiseResult() {
  const std::string in_path = TempPath("simple_lsp_hover_async_in.txt");
  const std::string out_path = TempPath("simple_lsp_hover_async_out.txt");
  const std::string err_path = TempPath("simple_lsp_hover_async_err.txt");
  const std::string uri = "file:///workspace/hover_async.simple";
  const std::string init_req =
      "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"initialize\",\"params\":{}}";
  const std::string open_req =
      "{\"jsonrpc\":\"2.0\",\"method\":\"textDocument/didOpen\",\"params\":{\"textDocument\":{"
      "\"uri\":\"" + uri + "\",\"languageId\":\"simple\",\"version\":1,"
      "\"text\":\"fetch :: async (value : i32) -> i32 { return value }\\nmain :: async () -> i32 { return await fetch(42) }\"}}}";
  const std::string hover =
      "{\"jsonrpc\":\"2.0\",\"id\":75,\"method\":\"textDocument/hover\",\"params\":{"
      "\"textDocument\":{\"uri\":\"" + uri + "\"},\"position\":{\"line\":1,\"character\":41}}}";
  const std::string shutdown_req =
      "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"shutdown\",\"params\":null}";
  const std::string exit_req =
      "{\"jsonrpc\":\"2.0\",\"method\":\"exit\",\"params\":null}";
  if (!WriteBinaryFile(in_path, BuildLspFrame(init_req) + BuildLspFrame(open_req) +
      BuildLspFrame(hover) + BuildLspFrame(shutdown_req) + BuildLspFrame(exit_req))) {
    return false;
  }
  if (!RunCommand(LspPipeCommand(in_path, out_path, err_path))) return false;
  const std::string output = ReadFileText(out_path);
  return ReadFileText(err_path).empty() && output.find("\"id\":75") != std::string::npos &&
         output.find("fetch : async (value : i32) -> Promise<i32>") != std::string::npos;
}
...
bool LspHoverShowsExternFunctionSignatureFromOpenModule() {
  ...
  const std::string open_lib =
      ...
      "\"text\":\"module Raylib\\nextern ffi.LoadRenderTexture :: (width : i32, height : i32) -> RenderTexture2D\"}}}";
  ...
  return ReadFileText(err_path).empty() &&
         ...
         out_contents.find("LoadRenderTexture : (width : i32, height : i32) -> RenderTexture2D") != std::string::npos &&
...
bool LspRaylibNamespaceWrapperFactsDriveHoverSignatureAndInlay() {
  ...
  const std::string open_lib =
      ...
      "\"uri\":\"" + lib_uri + "\",\"languageId\":\"simple\",\"version\":1,"
      "\"text\":\"module Raylib\\n"
      "extern raylib.DrawText : (text : string, posX : i32, posY : i32, fontSize : i32, color : Color) -> void\\n"
      "Raylib :: namespace {\\n"
      "  DrawText : (text : string, posX : i32, posY : i32, fontSize : i32, color : Color) -> void { lib.DrawText(text, posX, posY, fontSize, color) }\\n"
      "}\"}}}";
  const std::string open_main =
      "{\"jsonrpc\":\"2.0\",\"method\":\"textDocument/didOpen\",\"params\":{\"textDocument\":{"
      "\"uri\":\"" + main_uri + "\",\"languageId\":\"simple\",\"version\":1,"
  ...
  const std::string out_contents = ReadFileText(out_path);
  return ReadFileText(err_path).empty() &&
         out_contents.find("Raylib.DrawText : (text : string, posX : i32, posY : i32, fontSize : i32, color : Color) -> void") != std::string::npos &&
...
bool LspNamespaceMemberBasicsUseQualifiedNames() {
  ...
  const std::string err_path = TempPath("simple_lsp_namespace_member_err.txt");
  const std::string uri = "file:///workspace/namespace_member.simple";
  const std::string text =
      "Math :: namespace {\\n"
      "  add : (lhs : i32, rhs : i32) -> i32 { return lhs; }\\n"
      "}\\n"
      "main : () -> i32 { return Math.add(1, 2); }";
  const std::string init_req = "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"initialize\",\"params\":{}}";
  ...
  const std::string hover_ns = "{\"jsonrpc\":\"2.0\",\"id\":87,\"method\":\"textDocument/hover\",\"params\":{\"textDocument\":{\"uri\":\"" + uri + "\"},\"position\":{\"line\":0,\"character\":1}}}";
  const std::string hover_decl = "{\"jsonrpc\":\"2.0\",\"id\":88,\"method\":\"textDocument/hover\",\"params\":{\"textDocument\":{\"uri\":\"" + uri + "\"},\"position\":{\"line\":1,\"character\":3}}}";
  const std::string hover_call = "{\"jsonrpc\":\"2.0\",\"id\":89,\"method\":\"textDocument/hover\",\"params\":{\"textDocument\":{\"uri\":\"" + uri + "\"},\"position\":{\"line\":3,\"character\":32}}}";
  const std::string sig_req = "{\"jsonrpc\":\"2.0\",\"id\":90,\"method\":\"textDocument/signatureHelp\",\"params\":{\"textDocument\":{\"uri\":\"" + uri + "\"},\"position\":{\"line\":3,\"character\":39}}}";
  const std::string tokens_req = "{\"jsonrpc\":\"2.0\",\"id\":91,\"method\":\"textDocument/semanticTokens/full\",\"params\":{\"textDocument\":{\"uri\":\"" + uri + "\"}}}";
  const std::string inlay_req = "{\"jsonrpc\":\"2.0\",\"id\":92,\"method\":\"textDocument/inlayHint\",\"params\":{\"textDocument\":{\"uri\":\"" + uri + "\"},\"range\":{\"start\":{\"line\":3,\"character\":0},\"end\":{\"line\":3,\"character\":50}}}}";
  const std::string shutdown_req = "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"shutdown\",\"params\":null}";
  const std::string exit_req = "{\"jsonrpc\":\"2.0\",\"method\":\"exit\",\"params\":null}";
  if (!WriteBinaryFile(in_path, BuildLspFrame(init_req) + BuildLspFrame(open_req) + BuildLspFrame(hover_ns) + BuildLspFrame(hover_decl) + BuildLspFrame(hover_call) + BuildLspFrame(sig_req) + BuildLspFrame(inlay_req) + BuildLspFrame(tokens_req) + BuildLspFrame(shutdown_req) + BuildLspFrame(exit_req))) return false;
  if (!RunCommand(LspPipeCommand(in_path, out_path, err_path))) return false;
  const std::string out_contents = ReadFileText(out_path);
  std::vector<SemanticTokenEntry> entries;
  if (!ReadFileText(err_path).empty() || !DecodeSemanticData(out_contents, &entries)) return false;
  bool saw_ns = false;
  bool saw_call_fn = false;
  for (const auto& entry : entries) {
    if (entry.line == 0 && entry.col == 0 && entry.len == 4 && entry.type == 7 && (entry.modifiers & 1)) saw_ns = true;
    if (entry.line == 3 && entry.col == 31 && entry.len == 3 && entry.type == 2) saw_call_fn = true;
  }
  return out_contents.find("Math :: namespace") != std::string::npos &&
         out_contents.find("Math.add : (lhs : i32, rhs : i32) -> i32") != std::string::npos &&
         out_contents.find("\"id\":90") != std::string::npos &&
         out_contents.find("\"id\":92") != std::string::npos &&
         out_contents.find("\"label\":\"lhs: \"") != std::string::npos &&
         out_contents.find("\"label\":\"rhs: \"") != std::string::npos &&
         out_contents.find("\"label\":\": i32\"") != std::string::npos && saw_ns && saw_call_fn;
}
...
bool LspHoverShowsIoAliasSignature() {
  ...
  const std::string open_req =
      "{\"jsonrpc\":\"2.0\",\"method\":\"textDocument/didOpen\",\"params\":{\"textDocument\":{"
      "\"uri\":\"" + uri + "\",\"languageId\":\"simple\",\"version\":1,"
      "\"text\":\"import Standard.IO as Out\\nOut.println(\\\"hi\\\");\"}}}";
  const std::string hover_req =
...
bool LspHoverShowsFunctionSignature() {
  ...
  const std::string open_req =
      "{\"jsonrpc\":\"2.0\",\"method\":\"textDocument/didOpen\",\"params\":{\"textDocument\":{"
      "\"uri\":\"" + uri + "\",\"languageId\":\"simple\",\"version\":1,"
      "\"text\":\"sum : (lhs : i32, rhs : i32) -> i32 { return lhs + rhs; }\\n"
      "value : i32 = sum(1, 2);\"}}}";
  const std::string hover_req =
      "{\"jsonrpc\":\"2.0\",\"id\":46,\"method\":\"textDocument/hover\",\"params\":{"
...
bool LspCompletionReturnsItems() {
  ...
  return err_contents.empty() &&
         ...
         out_contents.find("Standard.IO.println") != std::string::npos;
}
...
bool LspCompletionSuggestsReservedImportModules() {
  ...
  return err_contents.empty() &&
         ...
         out_contents.find("\"label\":\"Standard.Log\"") != std::string::npos &&
         out_contents.find("\"label\":\"import\"") == std::string::npos;
}
...
bool LspCompletionIncludesManagedPromiseControls() {
  const std::string session_name = "simple_lsp_completion_managed_promise_controls";
  const std::string uri = "file:///workspace/complete_managed_promise.simple";
  const std::string init_req =
      "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"initialize\",\"params\":{}}";
  const std::string open_req =
      "{\"jsonrpc\":\"2.0\",\"method\":\"textDocument/didOpen\",\"params\":{\"textDocument\":{"
      "\"uri\":\"" + uri + "\",\"languageId\":\"simple\",\"version\":1,"
      "\"text\":\"work :: async () -> i32 { return 42 }\\nmain :: async () -> i32 {\\n  promise : Promise<i32> = work()\\n  promise.is\\n}\"}}}";
  const std::string completion_req =
      "{\"jsonrpc\":\"2.0\",\"id\":61,\"method\":\"textDocument/completion\",\"params\":{"
      "\"textDocument\":{\"uri\":\"" + uri + "\"},\"position\":{\"line\":3,\"character\":12}}}";
  const std::string shutdown_req =
      "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"shutdown\",\"params\":null}";
  const std::string exit_req =
      "{\"jsonrpc\":\"2.0\",\"method\":\"exit\",\"params\":null}";
  const std::string input = BuildLspFrame(init_req) + BuildLspFrame(open_req) +
      BuildLspFrame(completion_req) + BuildLspFrame(shutdown_req) + BuildLspFrame(exit_req);
  std::string output;
  std::string errors;
  if (!RunLspSession(session_name, input, &output, &errors)) return false;
  return errors.empty() && output.find("\"id\":61") != std::string::npos &&
      output.find("\"label\":\"isDone\"") != std::string::npos &&
      output.find("\"label\":\"isCancelled\"") != std::string::npos;
}
...
bool LspSignatureHelpReturnsSignature() {
  ...
  return err_contents.empty() &&
         ...
         out_contents.find("Standard.IO.println : (value) -> void") != std::string::npos &&
...
bool LspSignatureHelpTracksActiveParameter() {
  ...
  const std::string open_req =
      "{\"jsonrpc\":\"2.0\",\"method\":\"textDocument/didOpen\",\"params\":{\"textDocument\":{"
      "\"uri\":\"" + uri + "\",\"languageId\":\"simple\",\"version\":1,"
      "\"text\":\"import Standard.IO\\nStandard.IO.println(1, 2);\"}}}";
  const std::string signature_req =
      "{\"jsonrpc\":\"2.0\",\"id\":18,\"method\":\"textDocument/signatureHelp\",\"params\":{"
      "\"textDocument\":{\"uri\":\"" + uri + "\"},\"position\":{\"line\":1,\"character\":24}}}";
  const std::string shutdown_req = "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"shutdown\",\"params\":null}";
  const std::string exit_req = "{\"jsonrpc\":\"2.0\",\"method\":\"exit\",\"params\":null}";
  const std::string input =
      BuildLspFrame(init_req) +
...
bool LspSignatureHelpReturnsFormatSignature() {
  ...
  return err_contents.empty() &&
         ...
         out_contents.find("Standard.IO.println : (format, values...) -> void") != std::string::npos &&
         out_contents.find("\"activeSignature\":1") != std::string::npos &&
         out_contents.find("\"activeParameter\":1") != std::string::npos;
}
...
bool LspSignatureHelpSupportsAtCastCalls() {
  ...
  const std::string open_req =
      "{\"jsonrpc\":\"2.0\",\"method\":\"textDocument/didOpen\",\"params\":{\"textDocument\":{"
      "\"uri\":\"" + uri + "\",\"languageId\":\"simple\",\"version\":1,"
      "\"text\":\"main : () -> i32 { x : i8 = 1; return @i32(x); }\"}}}";
  const std::string signature_req =
      "{\"jsonrpc\":\"2.0\",\"id\":29,\"method\":\"textDocument/signatureHelp\",\"params\":{"
      "\"textDocument\":{\"uri\":\"" + uri + "\"},\"position\":{\"line\":0,\"character\":47}}}";
  const std::string shutdown_req = "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"shutdown\",\"params\":null}";
  const std::string exit_req = "{\"jsonrpc\":\"2.0\",\"method\":\"exit\",\"params\":null}";
  const std::string input =
      BuildLspFrame(init_req) +
...
bool LspSignatureHelpForLocalFunctionDeclaration() {
  ...
  const std::string open_req =
      "{\"jsonrpc\":\"2.0\",\"method\":\"textDocument/didOpen\",\"params\":{\"textDocument\":{"
      "\"uri\":\"" + uri + "\",\"languageId\":\"simple\",\"version\":1,"
      "\"text\":\"add : (a : i32, b : i32) -> i32 { return a + b; }\\nmain : () -> i32 { return add(1, 2); }\"}}}";
  const std::string signature_req =
  ...
  return err_contents.empty() &&
         ...
         out_contents.find("add : (a : i32, b : i32) -> i32") != std::string::npos &&
...
bool LspSignatureHelpForReservedAliasMember() {
  ...
  return err_contents.empty() &&
         ...
         out_contents.find("OS.sleepMs : (milliseconds) -> void") != std::string::npos &&
...
bool LspSignatureHelpForIoAliasFormatCall() {
  ...
  const std::string open_req =
      ...
      "\"text\":\"import Standard.IO as Out\\nOut.println(\\\"x={}\\\", 1);\"}}}";
  const std::string signature_req =
      "{\"jsonrpc\":\"2.0\",\"id\":36,\"method\":\"textDocument/signatureHelp\",\"params\":{"
      "\"textDocument\":{\"uri\":\"" + uri + "\"},\"position\":{\"line\":1,\"character\":23}}}";
  const std::string shutdown_req = "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"shutdown\",\"params\":null}";
  const std::string exit_req = "{\"jsonrpc\":\"2.0\",\"method\":\"exit\",\"params\":null}";
  const std::string input =
      BuildLspFrame(init_req) +
  ...
  return err_contents.empty() &&
         ...
         out_contents.find("Out.println : (format, values...) -> void") != std::string::npos &&
         out_contents.find("\"activeSignature\":1") != std::string::npos &&
         out_contents.find("\"activeParameter\":1") != std::string::npos;
}
...
bool LspSignatureHelpForCoreDlOpenOverloads() {
  ...
  return err_contents.empty() &&
         ...
         out_contents.find("FFI.open : (path) -> i64") != std::string::npos &&
         out_contents.find("FFI.open : (path, manifest) -> i64") != std::string::npos &&
         out_contents.find("\"activeSignature\":1") != std::string::npos &&
         out_contents.find("\"activeParameter\":1") != std::string::npos;
}
...
bool LspSignatureHelpForPromiseRun() {
  ...
  return err_contents.empty() && out_contents.find("\"id\":61") != std::string::npos &&
         out_contents.find("Promise.run : (delayMs, result) -> i64") != std::string::npos &&
         out_contents.find("\"activeParameter\":1") != std::string::npos;
}
...
bool LspSemanticTokensClassifyNamespaceExternDeclarationParts() {
  ...
  const std::string open_req =
      "{\"jsonrpc\":\"2.0\",\"method\":\"textDocument/didOpen\",\"params\":{\"textDocument\":{"
      "\"uri\":\"" + uri + "\",\"languageId\":\"simple\",\"version\":1,"
      "\"text\":\"Raylib :: namespace {\\n  extern InitWindow : (width : i32) -> void\\n}\"}}}";
  const std::string tokens_req =
  ...
  for (const auto& entry : entries) {
    ...
    if (entry.line == 1 && entry.col == 39 && entry.len == 4 && entry.type == 1) saw_return_type = true;
  }
...
bool LspSemanticTokensClassifyUsingKeyword() {
  ...
  const std::string err_path = TempPath("simple_lsp_tokens_using_err.txt");
  const std::string uri = "file:///workspace/tokens_using.simple";
  const std::string init_req = "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"initialize\",\"params\":{}}";
  const std::string open_req =
      "{\"jsonrpc\":\"2.0\",\"method\":\"textDocument/didOpen\",\"params\":{\"textDocument\":{"
      "\"uri\":\"" + uri + "\",\"languageId\":\"simple\",\"version\":1,"
      "\"text\":\"using Standard.IO\\nmain : () -> i32 { return 0; }\"}}}";
  const std::string tokens_req =
  ...
  if (!RunCommand(LspPipeCommand(in_path, out_path, err_path))) return false;
  const std::string out_contents = ReadFileText(out_path);
  if (!ReadFileText(err_path).empty() || out_contents.find("\"id\":60") == std::string::npos) return false;
  std::vector<SemanticTokenEntry> entries;
  if (!DecodeSemanticData(out_contents, &entries)) return false;
  for (const auto& entry : entries) {
    if (entry.line == 0 && entry.col == 0 && entry.len == 5 && entry.type == 0) return true;
  }
  return false;
}
...
bool LspSemanticTokensClassifyImmutableVariablesAndParameters() {
  ...
  for (const auto& entry : entries) {
    ...
    if (entry.line == 1 && entry.col == 16 && entry.len == 1 && entry.type == 4 && (entry.modifiers & 1)) saw_immutable_param = true;
    if (entry.line == 1 && entry.col == 21 && entry.len == 3 && entry.type == 1) saw_immutable_param_type = true;
  }
...
bool LspSemanticTokensMemberCallIsFunction() {
  ...
  const std::string open_req =
      ...
      "\"text\":\"player.Update();\"}}}";
  const std::string tokens_req =
      "{\"jsonrpc\":\"2.0\",\"id\":68,\"method\":\"textDocument/semanticTokens/full\",\"params\":{"
      "\"textDocument\":{\"uri\":\"" + uri + "\"}}}";
  const std::string shutdown_req = "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"shutdown\",\"params\":null}";
  const std::string exit_req = "{\"jsonrpc\":\"2.0\",\"method\":\"exit\",\"params\":null}";
  const std::string input =
  ...
  std::vector<SemanticTokenEntry> entries;
  if (!DecodeSemanticData(out_contents, &entries)) return false;
  const size_t member_pos = std::string("player.Update();").find("Update");
  ...
  for (const auto& entry : entries) {
    if (entry.line == target_line &&
      ...
      found_type = entry.type;
      break;
    }
  }
  return out_contents.find("\"id\":68") != std::string::npos &&
         found_type == 2;
}
...
bool LspSemanticTokensClassifyEnumNameAsType() {
  ...
  const std::string open_req =
      ...
      "\"text\":\"" + open_text + "\"}}}";
  const std::string tokens_req =
      "{\"jsonrpc\":\"2.0\",\"id\":59,\"method\":\"textDocument/semanticTokens/full\",\"params\":{"
      "\"textDocument\":{\"uri\":\"" + uri + "\"}}}";
  const std::string shutdown_req = "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"shutdown\",\"params\":null}";
  const std::string exit_req = "{\"jsonrpc\":\"2.0\",\"method\":\"exit\",\"params\":null}";
  const std::string input =
...
bool LspSemanticTokensMarkAggregateFieldsAsProperties() {
  ...
  const std::string open_req =
      ...
      "\"text\":\"" + open_text + "\"}}}";
  const std::string tokens_req =
      "{\"jsonrpc\":\"2.0\",\"id\":61,\"method\":\"textDocument/semanticTokens/full\",\"params\":{"
      "\"textDocument\":{\"uri\":\"" + uri + "\"}}}";
  const std::string shutdown_req = "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"shutdown\",\"params\":null}";
  const std::string exit_req = "{\"jsonrpc\":\"2.0\",\"method\":\"exit\",\"params\":null}";
  const std::string input =
...
bool LspSemanticTokensCycleMemberAccessDepth() {
  ...
  auto token_type_at = [&](size_t col, int len) -> int {
    for (const auto& entry : entries) {
      if (entry.line == 0 &&
          entry.col == static_cast<int>(col) &&
          entry.len == len) {
        return entry.type;
      }
    }
  ...
  const int alpha_type = token_type_at(alpha_pos, 5);
  const int beta_type = token_type_at(beta_pos, 4);
  const int gamma_type = token_type_at(gamma_pos, 5);
...
bool LspSemanticTokensClassifyReservedAliasAsNamespace() {
  ...
  const std::string open_req =
      ...
      "\"text\":\"" + open_text + "\"}}}";
  const std::string tokens_req =
      "{\"jsonrpc\":\"2.0\",\"id\":63,\"method\":\"textDocument/semanticTokens/full\",\"params\":{"
      "\"textDocument\":{\"uri\":\"" + uri + "\"}}}";
  const std::string shutdown_req = "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"shutdown\",\"params\":null}";
  const std::string exit_req = "{\"jsonrpc\":\"2.0\",\"method\":\"exit\",\"params\":null}";
  const std::string input =
...
bool LspSemanticTokensMarkReservedAliasDefaultLibrary() {
  ...
  const std::string open_req =
      ...
      "\"text\":\"" + open_text + "\"}}}";
  const std::string tokens_req =
      "{\"jsonrpc\":\"2.0\",\"id\":64,\"method\":\"textDocument/semanticTokens/full\",\"params\":{"
      "\"textDocument\":{\"uri\":\"" + uri + "\"}}}";
  const std::string shutdown_req = "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"shutdown\",\"params\":null}";
  const std::string exit_req = "{\"jsonrpc\":\"2.0\",\"method\":\"exit\",\"params\":null}";
  const std::string input =
...
bool LspDeclarationReturnsLocation() {
  ...
  const std::string open_req =
      ...
      "\"uri\":\"" + uri + "\",\"languageId\":\"simple\",\"version\":1,"
      "\"text\":\"foo : i32 = 1;\\nbar : i32 = foo;\"}}}";
  const std::string decl_req =
      "{\"jsonrpc\":\"2.0\",\"id\":31,\"method\":\"textDocument/declaration\",\"params\":{"
      "\"textDocument\":{\"uri\":\"" + uri + "\"},\"position\":{\"line\":1,\"character\":12}}}";
  const std::string shutdown_req = "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"shutdown\",\"params\":null}";
  const std::string exit_req = "{\"jsonrpc\":\"2.0\",\"method\":\"exit\",\"params\":null}";
  const std::string input =
...
bool LspCodeActionInsertsAfterImports() {
  ...
  const std::string open_req =
      ...
      "\"text\":\"import Standard.IO\\ny = 1;\"}}}";
  const std::string action_req =
      "{\"jsonrpc\":\"2.0\",\"id\":32,\"method\":\"textDocument/codeAction\",\"params\":{"
      "\"textDocument\":{\"uri\":\"" + uri + "\"},"
...
bool LspCodeActionInfersFloatDeclarationType() {
  ...
  const std::string open_req =
      ...
      "\"text\":\"rate = 1.5;\"}}}";
  const std::string action_req =
      "{\"jsonrpc\":\"2.0\",\"id\":38,\"method\":\"textDocument/codeAction\",\"params\":{"
      "\"textDocument\":{\"uri\":\"" + uri + "\"},"
      "\"range\":{\"start\":{\"line\":0,\"character\":0},\"end\":{\"line\":0,\"character\":1}},"
      "\"context\":{\"diagnostics\":[{\"code\":\"E0001\"}]}}}";
  const std::string shutdown_req = "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"shutdown\",\"params\":null}";
...
bool LspTypeDefinitionProviderResolvesGenericTypeArguments() {
  const std::string in_path = TempPath("simple_lsp_type_generic_in.txt");
  const std::string out_path = TempPath("simple_lsp_type_generic_out.txt");
  ...
  const std::string init_req = "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"initialize\",\"params\":{}}";
  const std::string open_req = "{\"jsonrpc\":\"2.0\",\"method\":\"textDocument/didOpen\",\"params\":{\"textDocument\":{\"uri\":\"" + uri + "\",\"languageId\":\"simple\",\"version\":1,\"text\":\"" + text + "\"}}}";
  const std::string type_req = "{\"jsonrpc\":\"2.0\",\"id\":61,\"method\":\"textDocument/typeDefinition\",\"params\":{\"textDocument\":{\"uri\":\"" + uri + "\"},\"position\":{\"line\":2,\"character\":18}}}";
  const std::string shutdown_req = "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"shutdown\",\"params\":null}";
  const std::string exit_req = "{\"jsonrpc\":\"2.0\",\"method\":\"exit\",\"params\":null}";
  if (!WriteBinaryFile(in_path, BuildLspFrame(init_req) + BuildLspFrame(open_req) + BuildLspFrame(type_req) + BuildLspFrame(shutdown_req) + BuildLspFrame(exit_req))) return false;
  if (!RunCommand(LspPipeCommand(in_path, out_path, err_path))) return false;
  const std::string out_contents = ReadFileText(out_path);
  return ReadFileText(err_path).empty() && out_contents.find("\"id\":61") != std::string::npos &&
         out_contents.find("\"line\":0") != std::string::npos &&
         out_contents.find("\"character\":0") != std::string::npos;
}
...
bool LspDocumentLinkResolvesReservedImportDocs() {
  ...
  const std::string text = "import Standard.IO\nmain : () -> i32 { return 0 }";
  const std::string init_req = "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"initialize\",\"params\":{}}";
  const std::string open_req = "{\"jsonrpc\":\"2.0\",\"method\":\"textDocument/didOpen\",\"params\":{\"textDocument\":{\"uri\":\"" + uri + "\",\"languageId\":\"simple\",\"version\":1,\"text\":\"" + text + "\"}}}";
...
bool LspLinkedEditingRangeReturnsIdentifierRanges() {
  ...
  const std::string err_path = TempPath("simple_lsp_linked_editing_err.txt");
  const std::string uri = "file:///workspace/linked.simple";
  const std::string text = "main : () -> i32 {\\n  value : i32 = 1;\\n  return value;\\n}";
  const std::string init_req = "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"initialize\",\"params\":{}}";
  const std::string open_req = "{\"jsonrpc\":\"2.0\",\"method\":\"textDocument/didOpen\",\"params\":{\"textDocument\":{\"uri\":\"" + uri + "\",\"languageId\":\"simple\",\"version\":1,\"text\":\"" + text + "\"}}}";
  const std::string linked_req = "{\"jsonrpc\":\"2.0\",\"id\":49,\"method\":\"textDocument/linkedEditingRange\",\"params\":{\"textDocument\":{\"uri\":\"" + uri + "\"},\"position\":{\"line\":1,\"character\":3}}}";
  const std::string shutdown_req = "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"shutdown\",\"params\":null}";
  const std::string exit_req = "{\"jsonrpc\":\"2.0\",\"method\":\"exit\",\"params\":null}";
  if (!WriteBinaryFile(in_path, BuildLspFrame(init_req) + BuildLspFrame(open_req) + BuildLspFrame(linked_req) + BuildLspFrame(shutdown_req) + BuildLspFrame(exit_req))) return false;
  if (!RunCommand(LspPipeCommand(in_path, out_path, err_path))) return false;
  const std::string out_contents = ReadFileText(out_path);
  return ReadFileText(err_path).empty() && out_contents.find("\"id\":49") != std::string::npos &&
         out_contents.find("\"ranges\":[") != std::string::npos &&
         out_contents.find("\"wordPattern\":\"[A-Za-z_][A-Za-z0-9_]*\"") != std::string::npos &&
         out_contents.find("\"character\":2") != std::string::npos &&
         out_contents.find("\"character\":9") != std::string::npos;
}
...
bool LspInlayHintReturnsParameterHints() {
  ...
  const std::string text = "add : (lhs : i32, rhs :: i32) -> i32 { return lhs + rhs }\\nmain : () -> i32 { return add(1, 2) }";
  const std::string init_req = "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"initialize\",\"params\":{}}";
  const std::string open_req = "{\"jsonrpc\":\"2.0\",\"method\":\"textDocument/didOpen\",\"params\":{\"textDocument\":{\"uri\":\"" + uri + "\",\"languageId\":\"simple\",\"version\":1,\"text\":\"" + text + "\"}}}";
...
bool LspCodeLensReturnsSimpleCommands() {
  const std::string in_path = TempPath("simple_lsp_codelens_in.txt");
  const std::string out_path = TempPath("simple_lsp_codelens_out.txt");
  const std::string err_path = TempPath("simple_lsp_codelens_err.txt");
  const std::string uri = "file:///workspace/codelens.simple";
  const std::string text = "helper : () -> i32 { return 1 }\\nmain : () -> i32 { return helper() }";
  const std::string init_req = "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"initialize\",\"params\":{}}";
  const std::string open_req = "{\"jsonrpc\":\"2.0\",\"method\":\"textDocument/didOpen\",\"params\":{\"textDocument\":{\"uri\":\"" + uri + "\",\"languageId\":\"simple\",\"version\":1,\"text\":\"" + text + "\"}}}";
  const std::string lens_req = "{\"jsonrpc\":\"2.0\",\"id\":45,\"method\":\"textDocument/codeLens\",\"params\":{\"textDocument\":{\"uri\":\"" + uri + "\"}}}";
  const std::string shutdown_req = "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"shutdown\",\"params\":null}";
  const std::string exit_req = "{\"jsonrpc\":\"2.0\",\"method\":\"exit\",\"params\":null}";
...
const TestCase kLspTests[] = {
  ...
  {"lsp_did_open_accepts_lambdas", LspDidOpenAcceptsLambdas},
  ...
  {"lsp_completion_includes_managed_promise_controls", LspCompletionIncludesManagedPromiseControls},
  ...
  {"lsp_signature_help_supports_at_cast_calls", LspSignatureHelpSupportsAtCastCalls},
  ...
  {"lsp_semantic_tokens_member_call_is_function", LspSemanticTokensMemberCallIsFunction},
  {"lsp_semantic_tokens_fallback_on_malformed_source", LspSemanticTokensFallbackOnMalformedSource},
  {"lsp_semantic_tokens_classify_import_alias_as_namespace",
   LspSemanticTokensClassifyImportAliasAsNamespace},
  {"lsp_semantic_tokens_classify_enum_members", LspSemanticTokensClassifyEnumMembers},
  ...
  {"lsp_rename_rejects_reserved_alias_member", LspRenameRejectsReservedAliasMember},
  ...
  {"lsp_prepare_rename_rejects_reserved_alias_member", LspPrepareRenameRejectsReservedAliasMember},
