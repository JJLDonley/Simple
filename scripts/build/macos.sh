...
if [[ "$SKIP_BUILD" -eq 0 ]]; then
  ...
  cp "$BIN_OUT/simple" "$ROOT_DIR/bin/simple"
...
if [[ "$SKIP_RELEASE" -eq 0 ]]; then
  ...
  for lib in "$BIN_OUT"/libsimplevm_runtime.dylib "$BIN_OUT"/libsimplevm_runtime.a "$BIN_OUT"/libsimplevm_core.dylib "$BIN_OUT"/libsimplevm_core.a; do
...
if [[ "$SKIP_INSTALL" -eq 0 ]]; then
  ...
  mkdir -p "$BIN_DIR"
  ln -sfn "${PREFIX}/current/bin/svm" "${BIN_DIR}/svm"
