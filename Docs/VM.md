...
`VM/include/runtime/abi.h` maps current SBC `TypeKind` values to ABI classes, sizes, and alignments. Canonical runtime type identities provide deterministic strings for primitive, reference, handle, stable aggregate, option, result, and promise type arguments used by future monomorphic generic specialization.
