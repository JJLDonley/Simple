...
`VM/include/runtime/abi.h` maps current SBC `TypeKind` values to ABI classes, sizes, and alignments. Canonical runtime type identities provide deterministic strings for primitive, enum, pointer, array/list, reference, handle, channel, stable aggregate, option, result, promise, and instantiated type arguments used by future monomorphic generic specialization. Generic symbol helpers produce human/debug names, escaped hash-suffixed link names, and collision diagnostics.
