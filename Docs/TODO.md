...
- [ ] `Lang/src/lang_validate.cpp` is not a semantic monolith.
  ...
  - Progress: pure `TypeRef` cloning, structural type equality, and primitive/scalar type-name classification moved to `TAST/types`; literal shape classification moved to `TAST/literals`; both have direct phase-owned tests.
