...
- [ ] `Lang/src/lang_validate.cpp` is not a semantic monolith.
  ...
  - Progress: declaration member lookup/listing moved to `RAST/member_resolution`; reserved native-module, member-list, IO print-name, and module-expression helpers are owned by `RAST/reserved_resolution`; pure `TypeRef` construction, cloning/vector cloning, container element cloning, structural type equality, primitive/scalar type-name classification, and list method-name classification moved to `TAST/types`; literal shape classification, format placeholder counting, scalar literal compatibility, and expression/type compatibility moved to `TAST/literals`; generic parameter collection, generic type substitution, generic unification, and artifact generic maps moved to `TAST/generics`; procedure/function argument-count checks moved to `TAST/calls`; addressable expression shape plus scalar and compound-assignment operator checks moved to `TAST/expressions`; assignment-operator classification moved to `TAST/statements`; these have direct phase-owned tests.
