...
- [ ] `Lang/src/lang_validate.cpp` is not a semantic monolith.
  ...
  - Progress: pure `TypeRef` cloning/vector cloning, structural type equality, and primitive/scalar type-name classification moved to `TAST/types`; literal shape classification, scalar literal compatibility, and expression/type compatibility moved to `TAST/literals`; both have direct phase-owned tests.
