...
- [ ] Tests are split by owning subsystem/phase.
  ...
  - Progress: module-header CLI check, basic `svm emit/check/build` command tests, compile/build executable tests, `simple` runtime-stub command contract tests, CLI diagnostic-format checks, exit-code checks, and CLI import command regressions moved to `Tests/tests/cli/`.
