...
- [ ] `Lang/src/lang_validate.cpp` is not a semantic monolith.
  ...
  - Progress: pure `TypeRef` cloning/vector cloning, structural type equality, and primitive/scalar type-name classification moved to `TAST/types`; literal shape classification, format placeholder counting, scalar literal compatibility, and expression/type compatibility moved to `TAST/literals`; generic type substitution, generic unification, and artifact generic maps moved to `TAST/generics`; procedure/function argument-count checks moved to `TAST/calls`; scalar and compound-assignment operator checks moved to `TAST/expressions`; these have direct phase-owned tests.
