...
The ORC backend caches generated entries by module/function/code hash and verifies generated IR before installation. Current LLVM lowering covers scalar functions with constants including validated 128-bit placeholder constants, locals, uninitialized and numeric f32/f64-initialized globals in non-calling functions, integer/unsigned/floating arithmetic, checked i32/u32/i64/u64 arithmetic/div/mod, checked integer/float scalar conversions, checked/guard bounds, simple pointer guards/comparisons and pseudo-memory, trap/intrinsic-trap/syscall/throw/panic fallback, address/capture-local/global, algebraic wrapper, iterator placeholders, atomic placeholder, vector placeholder ops, comparisons, simple conversions, forward branches, `JmpTable`, loop state merging for validated scalar/ref-stack cases, scalar-safe direct Simple calls inside loops, including void methods encoded as unspecified returns, scalar/void helper-bridged import calls inside loops, `Yield`, selected intrinsics, self calls, no-capture function literals via `NewClosure` + `CallIndirect`, and helper-bridged direct/import/native/indirect non-self calls. Loop-call safety diagnostics are scoped to calls that are actually inside detected backward-branch ranges, so pre-loop setup imports no longer block render/update loop lowering. Native import calls inside loops require safe metadata: matching signature, non-blocking, no allocation, no GC safepoint, and no output/mutating resources; managed string/ref arguments and borrowed resource inputs are allowed through the helper ABI when those metadata checks pass. Dynamic `System.FFI.call$...` calls pass the canonical VM ABI/native ABI FFI verifier. Calls inside LLVM loops reject before machine-code execution and fall back to the interpreter because the external lifetime boundary is not represented in LLVM loop state; supported calls outside loops may use generic helper dispatch. Library-specific dynamic-DL direct binds are intentionally not part of LLVM lowering; aggregate external-C calls remain behind the canonical VM ABI/native ABI marshaler until generic aggregate support is proven. The LLVM cache/helper ABI versions are part of the cache key and are bumped when helper signatures or JIT acceptance rules change. Loop-call rejection diagnostics include the opcode PC, call category, target label, SBC signature shape, and aggregate field layout fingerprints so remaining FFI blockers can be mapped to concrete ABI forms. `System.Job` and `Standard.Promise` imports are interpreter-only in `v0.5.2` so each worker and promise remains owned by the executing VM's resource registry. Language `future.make` and typed `await` likewise reject LLVM before function entry with `unsupported: async Promise execution needs interpreter scheduler`; default execution then uses the rooted cooperative interpreter scheduler rather than compiling placeholder task identities. Resource/allocating direct/import calls, managed-result imports, managed direct calls, and indirect/procedure calls inside loops are still conservatively rejected until full caller-frame root publication, exact target metadata, and safepoint state are hardened. Functions whose own parameters include managed references, including artifact method receivers, also remain interpreter-owned; LLVM must reject them before execution rather than risk stale receiver state across branches or helper calls. Closures with one or more lexical captures likewise fall back before execution because their rooted capture-cell environment is not yet part of the LLVM entry ABI. `--jit-stats` prints function names with indexes, per-function interpreted opcode counts, dispatch/compiled execution counts, and rejection reasons including opcode/pc plus call category/safety reason and target labels where available. A static LLVM rejection is cached per function, so later calls execute through the interpreter without repeating JIT dispatch. Mixed workloads, such as raylib loops, can therefore identify the exact hot function, callee, and bytecode operation that fell back.
